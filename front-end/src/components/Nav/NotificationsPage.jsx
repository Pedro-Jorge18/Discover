...
function NotificationsPage({ user, setUser, onOpenSettings, onOpenSettingsAdmin }) {
  ...
  const unreadLabel = `${t('hostNotifications.unread')}: ${unreadCount}`;
  ...
  const handleClearAll = () => {
    clearHostNotifications(user.id);
    refresh();
  };

  const handleMarkSingle = (id) => {
    markSingleNotificationRead(user.id, id);
    refresh();
  };

  const handleRemoveSingle = (id) => {
    removeHostNotification(user.id, id);
    refresh();
  };
      ...
      <main className="max-w-[1100px] mx-auto px-5 sm:px-10 pt-44 pb-20">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-4xl font-black italic uppercase tracking-tighter text-gray-900">{t('hostNotifications.pageTitle')}</h1>
              <p className="text-sm text-gray-500 font-medium">{t('hostNotifications.pageSubtitle')}</p>
            </div>
          </div>
          <button
          ...
          >
            {t('hostNotifications.viewDashboard')}
          </button>
        </div>

        <div className="flex items-center gap-3 mb-6 text-sm">
          <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 font-semibold">{unreadLabel}</span>
          <button onClick={handleMarkAllRead} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-900 text-white text-xs font-bold uppercase tracking-wide hover:bg-gray-800">
            <CheckCheck className="w-4 h-4" /> {t('hostNotifications.markAllRead')}
          </button>
          <button onClick={handleClearAll} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 text-gray-700 text-xs font-bold uppercase tracking-wide hover:bg-gray-50">
            <Trash2 className="w-4 h-4" /> {t('hostNotifications.clearAll')}
          </button>
        </div>
              ...
              <div key={item.id} className="border border-gray-100 rounded-2xl p-4 flex items-start gap-3 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!item.read && <span className="px-2 py-1 text-[10px] font-bold uppercase tracking-wide bg-blue-50 text-blue-700 rounded-full">{t('hostNotifications.badgeNew')}</span>}
                  </div>
                ...
                <div className="flex flex-col gap-2 ml-2">
                  ...
                  <button
                    ...
                    title={t('hostNotifications.delete')}
                  >
