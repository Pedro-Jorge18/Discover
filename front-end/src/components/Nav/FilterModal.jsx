    ...
    <div className="fixed inset-0 z-110 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-slideUp text-left">
        ...
        <div className="p-10 space-y-12 overflow-y-auto max-h-[70vh]">
          <div>
            <h3 className="text-lg font-black uppercase tracking-widest mb-6 italic">Gama de Preço</h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {types.map((t) => (
                <button key={t.label} onClick={() => setType(t.label)} className={`flex flex-col items-center gap-3 p-6 rounded-3xl border-2 transition-all ${type === t.label ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-100'}`}>
                  {t.icon}
        ...
        <div className="p-8 bg-gray-50 flex justify-between items-center">
          <button onClick={() => { setPrice(1000); setType('Todos'); setSelectedExtras([]); }} className="text-gray-400 font-black uppercase text-xs tracking-widest underline">Limpar Filtros</button>
          <button onClick={handleApply} className="bg-blue-600 text-white px-12 py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl">Aplicar Filtros</button>
        </div>
      </div>
    </div>
  );
